...
inline RebasedRowSpan find_rebased_row_span(
    const std::vector<int>& row_ptr,
    int interval_begin,
    int interval_end) {
    const auto row_begin_it =
        std::upper_bound(row_ptr.begin() + 1, row_ptr.end(), interval_begin);
    const auto row_end_it =
        std::lower_bound(row_ptr.begin(), row_ptr.end() - 1, interval_end);
    return RebasedRowSpan{
        static_cast<int>(std::distance(row_ptr.begin(), row_begin_it)) - 1,
        static_cast<int>(std::distance(row_ptr.begin(), row_end_it))};
}

template <typename EmitFn>
void emit_rebased_row_offsets(
    const std::vector<int>& row_ptr,
    int interval_begin,
    int interval_end,
    RebasedRowSpan row_span,
    EmitFn&& emit) {
    for (int row = row_span.row_begin; row <= row_span.row_end; ++row) {
        const int clamped =
            std::clamp(row_ptr[static_cast<size_t>(row)], interval_begin, interval_end);
        emit(clamped - interval_begin);
    }
}
