...
void bind_atomic_module(py::module& m) {
    ...
    py::class_<ImageContainer<double>>(m, "ImageContainer")
        .def(py::init<AtomicData*>())
        .def("assemble", &ImageContainer<double>::assemble)
        .def("add_block", [](ImageContainer<double>& self, std::vector<int> R, int g_row, int g_col, py::array_t<double> data, AssemblyMode mode) {
            ...
            if (info.strides[0] == sizeof(double) && info.strides[1] == sizeof(double) * rows) {
                layout = MatrixLayout::ColMajor;
            }
            
            self.add_block(R, g_row, g_col, static_cast<double*>(info.ptr), rows, cols, mode, layout);
            
        }, py::arg("R"), py::arg("g_row"), py::arg("g_col"), py::arg("data"), py::arg("mode") = AssemblyMode::ADD)
        
        .def("add_blocks", [](ImageContainer<double>& self, 
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                ptrs[i] = static_cast<double*>(info.ptr);
                rows[i] = info.shape[0];
                cols[i] = info.shape[1];
                
                layouts[i] = MatrixLayout::RowMajor;
                if (info.strides[0] == sizeof(double) && info.strides[1] == (long)(sizeof(double) * rows[i])) {
        ...
        .def("sample_k", [](ImageContainer<double>& self, py::array_t<double> k_points, PhaseConvention convention) {
