...
-   **Padding invariant**: lanes `[num_vectors, ld)` of every multivector row are always zero. What depends on it is the **apply**: the dense kernels run the full `ld` width so there is no vec-axis tail path, and that is only correct while the pad lanes multiply to exact zeros. The flat element-wise ops (`scale`, `axpy`, `axpby`, `pointwise_mult`, `copy_from`) do sweep the padded buffer, but lane $j$ only touches lane $j$, so pad garbage could never reach a live lane; `bdot` is bounded by `num_vectors` and does not touch the pad at all. Any code that **bulk-writes the raw buffer** must re-establish the invariant via `zero_padding()` — no such writer exists today (every writer is lane-bounded, and the Python buffer protocol is strided over `ld`, so numpy cannot reach a pad lane), so `zero_padding()` is a hook for future ones and not something a caller should need.

Enforced by `DistMultiVector::assert_padding_zero()`, asserted as a pre- **and** postcondition on `mult_dense` / `mult_dense_adjoint`: the precondition catches a caller that bulk-wrote without repairing, the postcondition catches a kernel that stopped preserving. Both compile away under `NDEBUG`, so they cost nothing in a Release build — and are therefore only as good as the tests that `#undef NDEBUG`. `test_migration_contract` does, and covers both the apply round trip and a deliberate corruption, the latter so the former cannot pass vacuously. The predicate `padding_is_zero()` is not `NDEBUG`-gated and can be called from any build.
